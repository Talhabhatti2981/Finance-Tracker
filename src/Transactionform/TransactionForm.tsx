...
type Props = {
  addTransaction: (
    transaction: Omit<Transaction, "id"> & { amount: string | number }
  ) => void;
...
export default function TransactionForm({
  ...
  const validateField = (
    name: keyof Omit<Transaction, "id">,
    value: string
  ) => {
  ...
  const handleChange = (
    ...
    setErrors({
      ...errors,
      [name]: validateField(name as keyof Omit<Transaction, "id">, value),
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    (Object.keys(formData) as (keyof Omit<Transaction, "id">)[]).forEach(
      (key) => {
  ...
  return (
    <div className="p-6 min-h-screen max-w-3xl mx-auto">
      <motion.form
        ...
        className={`shadow-lg p-6 rounded-2xl space-y-6 ${formBg} border ${
      ...
      >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {["title", "amount", "date"].map((field, index) => (
            <motion.div
              key={field}
